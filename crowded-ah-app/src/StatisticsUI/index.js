...
const StatisticsUI = () => {
    ...
    const options = {
        ...
        scales: {
            x: {
                title: {
                    display: true,
                    text: 'Time of Day', // Updated X-axis label
                    font: {
                        size: 16
                    }
                }
        ...
        maintainAspectRatio: false, // Ensures chart doesn't resize poorly
    };
    ...
    const fetchAndProcessLocalCSV = async () => {
        try {
            ...
            // Step 2: Parse the CSV content using PapaParse
            Papa.parse(csvContent, {
                ...
                complete: (results) => {
                    ...
                    // Step 3: Group and process data by station and hour
  ...
  const fetchAndProcessCSV = async () => {
    ...
    <div className="chart-section-wrapper">
        <button className="navigation-button left-button" onClick={previousStation} disabled={currentStationIndex === 0}>
            Back
        </button>
        ...
        <button className="navigation-button right-button" onClick={nextStation} disabled={currentStationIndex === stations.length - 1}>
            Next
        </button>
    </div>
)}
